"""`plans` table (《数据库表结构与迁移规范》七)."""

from __future__ import annotations

from datetime import datetime
from typing import Any

from sqlalchemy import Column, Index
from sqlalchemy import JSON as SA_JSON
from sqlalchemy import Text
from sqlmodel import Field, SQLModel

from app.models.base import utcnow


class Plan(SQLModel, table=True):
    __tablename__ = "plans"
    __table_args__ = (
        Index("idx_plans_user_created", "user_id", "created_at"),
        Index("idx_plans_user_date", "user_id", "start_date", "end_date"),
    )

    id: str = Field(primary_key=True)
    user_id: str = Field(index=True, nullable=False)
    trip_id: str | None = Field(default=None, foreign_key="trips.id", index=True)
    location: str = Field(nullable=False)
    start_date: str | None = Field(default=None, nullable=True)
    end_date: str | None = Field(default=None, nullable=True)
    date_label: str = Field(nullable=False)
    content: str = Field(sa_column=Column(Text, nullable=False))
    # Full ItineraryData (see 《数据结构与通信接口规范》1.9).
    itinerary_data: dict[str, Any] = Field(
        default_factory=dict, sa_column=Column(SA_JSON, nullable=False)
    )
    created_at: datetime = Field(default_factory=utcnow, nullable=False)
    updated_at: datetime = Field(default_factory=utcnow, nullable=False)
