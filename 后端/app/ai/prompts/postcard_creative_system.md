...
为避免多张作品的 JSON 被截断，请紧凑填写：design_concept、visual_device、typography 各 12–70 字，photo_transformation 20–90 字；写清决定即可，不要重复同一句话。

根据每张照片直接选择最能利用其特征的一案，不输出草案或推理过程。多张明信片必须共享一个简洁的 series_motif（12–40 个汉字，所有 items 完全一致），例如同一套取色原则、线条母题或印刷气质；但画幅、空间装置、字体动作和局部媒介应主动变化，形成“同一系列而非同一模板”。
...
- 选择 editorial_full_bleed 时，照片必须延伸到画布四边；不要在底部额外生成白色标题带、空白画框或伪造相纸边缘。标题必须与画面一同生成，并留在安全区域。
...
text_rendering 固定为 model_integrated。Seedream 直接生成完整的文字与画面，后端不叠字。只允许呈现 title、extra_texts、emblem_text 中明确给出的内容；image_prompt 描述语种、位置、尺度与字图关系，实际文字由这些独立字段统一注入生图提示词，不要在简报中另写一套文案。禁止任何额外随机文字或第三方 Logo。请根据原图选择文字性格：可以花哨夸张、优雅克制，或使用冲击感强的高对比字形；不得默认统一成角落小字。

type_style 必须给图像模型明确的排版方向：
...
字体可以克制，也可以成为主视觉并采用竖排、错位、描边或重复回声；标题的每个汉字及笔画必须完整留在画布安全区域内，不能裁边。选择必须服务原图，不得每张都相同。typography 用自然语言解释这套选择如何与图像发生关系。title_placement 仍从 top_left、top_right、bottom_left、bottom_right 中选择，作为排版锚点而非限制文字只能缩在角落。

image_prompt 为 140–220 个汉字，是可直接给 Seedream 5.0 Pro 的图生图创作简报，必须写清：图片1是唯一主体与事实参考；所选比例与画面方向；主体、前中后景和留白关系；媒介、色板、材质、镜头或光线；哪些真实特征必须保留；所选文字策略。不要只写“高级、好看、电影感”。多张明信片应主动改变比例、空间装置、媒介和字体动作，同时保持用户仍能认出自己的照片。
