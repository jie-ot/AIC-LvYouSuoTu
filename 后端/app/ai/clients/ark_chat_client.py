"""OpenAI-compatible chat client for Ark tasks and DeepSeek planning.

Non-planning tasks use Ark Agent Plan; planning uses DeepSeek's own endpoint.
The historical module name is retained for internal imports only.
"""
...
def _resolve_runtime(planning_model: PlanningModel | None = None) -> ChatRuntime:
    if planning_model is None:
        return ChatRuntime(
            ...
            extra_body=_ARK_THINKING_EXTRA_BODY,
            uses_temperature=True,
            token_parameter="max_completion_tokens",
        )
    if planning_model not in ("deepseek-v4-flash", "deepseek-v4-pro"):
        raise InternalError("旅行规划仅支持 DeepSeek V4 Flash / Pro")
    selected = planning_model
    api_model = (
...
def _get_client(runtime: ChatRuntime):  # noqa: ANN202
    ...
    if not runtime.api_key:
        key_name = "DEEPSEEK_API_KEY" if runtime.provider == "deepseek" else "ARK_PLAN_API_KEY"
...
def _text_timeout_seconds(*, task: str | None = None, override: int | None = None) -> int:
    """Resolve per-call HTTP timeout for chat.completions."""
    if override is not None:
        return override
    if task in (TASK_PLANNING, TASK_PLANNING_INTAKE):
        return settings.DEEPSEEK_PLANNING_TIMEOUT_SECONDS
    return settings.ARK_TEXT_TIMEOUT_SECONDS
...
def _real_chat_json(
    *,
    ...
    Non-planning tasks use Ark Agent Plan. Planning intake/plain calls use
    DeepSeek even when the caller omits the model. Only final content is returned.
    """
    from openai import (
        APIConnectionError,
        APITimeoutError,
    ...
    selected = (
        (planning_model or DEFAULT_PLANNING_MODEL)
        if task in (TASK_PLANNING, TASK_PLANNING_INTAKE)
        else None
    )
    runtime = _resolve_runtime(selected)
    client = _get_client(runtime)
    messages = _build_messages(system_prompt, user_text, image_data_urls)
    attempts = max(1, settings.MODEL_MAX_RETRY + 1)
    last_transient: Exception | None = None
    ...
    while attempt < attempts:
        ...
        try:
            ...
            # Transient: retry up to MODEL_MAX_RETRY.
            last_transient = exc
            logger.warning(
                "model chat transient error (request_id=%s, attempt=%d/%d): %s",
                request_id, attempt + 1, attempts, type(exc).__name__,
        ...
        logger.info(
            "model chat ok task=%s request_id=%s latency_ms=%d",
...
def _chat_attempt_count(
    ...
    configured = max(1, settings.MODEL_MAX_RETRY + 1)
...
def _runtime_log_details(
    ...
    *,
    ...
    effective_thinking = runtime.provider == "ark" or (
        True if thinking_enabled is None else thinking_enabled
    )
    effective_effort = (
        None
        if not effective_thinking
        else (
            runtime.reasoning_effort if runtime.provider == "ark"
...
def _loggable_payload(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            ...
            if normalized_key in {"reasoning_content", "encrypted_content"}:
                result[str(key)] = (
...
def _sleep_before_retry(attempt: int, attempts: int) -> None:
    ...
    if attempt + 1 >= attempts:
        return
    base = max(0.0, settings.MODEL_RETRY_BACKOFF_SECONDS)
    if not base:
        return
