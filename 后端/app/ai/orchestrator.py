...
from app.ai.clients.ark_image_client import ImageGenerationResult
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            with timed_stage(
                ...
                call_kwargs: dict[str, Any] = {
                    "task": ark_chat_client.TASK_PHOTO_ANALYZE,
                ...
                raw = ark_chat_client.chat_json(**call_kwargs)
...
def _merge_overall_location(batches: list[PhotoAnalysisResult]) -> str | None:
    locations = [
        (batch.overall_location or "").strip()
        for batch in batches
        if (batch.overall_location or "").strip() not in _UNKNOWN_LOCATIONS
    ]
    if not locations:
        return "未知目的地"
    # Prefer the location repeated by independent batches; ties are stable and
    # deterministic instead of letting the first 5-photo batch dominate.
    counts = Counter(locations)
    return sorted(counts, key=lambda value: (-counts[value], locations.index(value), value))[0]
...
def _collect_date_candidates(
    ...
    dates: list[str] = []
    def add_if_valid(value: str | None) -> None:
        if not value:
            return
        try:
    ...
    for batch in batches:
        add_if_valid(batch.start_date)
        add_if_valid(batch.end_date)
    for item in photos:
        add_if_valid(item.taken_date_guess)
    return dates
    ...
    *,
    ...
    with timed_stage(
        ...
        raw = ark_chat_client.chat_json(
            task=ark_chat_client.TASK_POSTCARD_CREATIVE,
            system_prompt=system_prompt,
            user_text=user_text,
    ...
    *,
    ...
    try:
        ...
        log_event(
            ...
            sanitized_prompt_hash=hashlib.sha256(sanitized_prompt.encode("utf-8")).hexdigest()[:16],
...
def propose_memory_update(
    *,
    ...
    with timed_stage("orchestrator_memory_update_model", source_task=source_task):
        raw = ark_chat_client.chat_json(
            task=ark_chat_client.TASK_MEMORY_UPDATE,
            system_prompt=system_prompt,
    ...
    *,
    ...
    if execute_tool is None:
        with timed_stage(
            ...
            raw = ark_chat_client.chat_json(
                task=ark_chat_client.TASK_PLANNING_INTAKE,
                system_prompt=system_prompt,
                user_text=user_text,
                temperature=0.2,
    ...
    with timed_stage(
        ...
        for round_idx in range(_MAX_INTAKE_TOOL_ROUNDS):
            ...
            if not turn.tool_calls:
                ...
                try:
                    ...
                    repaired = ark_chat_client.chat_messages(
                        messages=messages,
                        tools=None,
        ...
        final = ark_chat_client.chat_messages(
            messages=messages,
            tools=None,
...
def _schedule_overlap_problems(
    ...
    for day in data.itinerary:
        ...
        previous = None
        previous_end_m: int | None = None
        for schedule in timed:
            start_m = parse_clock_minutes(schedule.start_time)
            if (
                previous is not None
                and previous_end_m is not None
                and start_m is not None
                and start_m < previous_end_m
            ):
                problems.append(
                    planning_feasibility.Problem(
                        message=(
            ...
            if schedule.end_time and _CLOCK_PATTERN.fullmatch(schedule.end_time):
                previous = schedule
                previous_end_m = timeline_end_minutes(
                    start_m, parse_clock_minutes(schedule.end_time)
                )
    return problems
        ...
        messages=[
            {"role": "system", "content": system_prompt},
            {
                ...
                "content": (
                    ...
                    "跨夜到达允许 end_time 钟点早于 start_time，不要改成错误的同日时刻；"
...
def _plan_plain(
    ...
    *,
    ...
    with timed_stage("planning_plain_model"):
        raw = ark_chat_client.chat_json(
            task=ark_chat_client.TASK_PLANNING,
            system_prompt=system_prompt,
            user_text=user_text,
            max_completion_tokens=_MAX_GENERATION_TOKENS,
            planning_model=planning_model,
    ...
    try:
        ...
        with timed_stage("planning_json_repair_model"):
            raw2 = ark_chat_client.chat_json(
                task=ark_chat_client.TASK_PLANNING,
                system_prompt=system_prompt,
