...
def attach_with_reference(
    ...
    *,
    ...
    new) reference. Missing, deleted or foreign assets are hard failures so a
    business record can never commit with a dangling reference.
    """
    asset = session.get(FileAsset, asset_id)
    if asset is None:
        raise InvalidParamError("文件资源不存在")
    if asset.user_id not in {user_id, SYSTEM_USER_ID}:
        raise InvalidParamError("文件资源不属于当前用户")
    if asset.status not in {STATUS_TEMPORARY, STATUS_ATTACHED}:
        raise InvalidParamError("文件资源状态不允许绑定")
    ...
    if existing is not None and existing.user_id != user_id:
        raise InvalidParamError("文件引用所属异常")
    if existing is None:
