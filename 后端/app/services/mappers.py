...
def postcard_to_dto(entity: PostcardEntity) -> dto.Postcard:
    return dto.Postcard(
        ...
        source_asset_ids=list(entity.source_asset_ids or []),
...
def report_to_dto(
    entity: ReportEntity, *, source_images: list[str] | None = None
) -> dto.Report:
    try:
        chart = [dto.ReportChartPoint.model_validate(point) for point in (entity.chart_data or [])]
    except Exception:  # Old malformed rows must not break the whole history list.
        chart = []
    try:
        profile_data = (
