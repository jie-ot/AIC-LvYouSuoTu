...
def build_checklist(brief: PlanningBrief) -> list[PlanningChecklistItem]:
    ...
    items = [
        ...
        _item(
            "travelers",
            "同行人数",
            f"{brief.traveler_count} 人" if brief.traveler_count else "未指定",
            required=False,
        ...
        _item(
            "lodging",
            "住宿偏好",
            brief.lodging_preference or "未指定",
            required=False,
...
def confirmed_requirement_text(brief: PlanningBrief, latest_message: str) -> str:
    ...
    return "\n".join(
        [
            ...
            + ("（用户未指定，生成时暂按 1 人安排）" if brief.traveler_count is None else ""),
            f"- 预算：{brief.budget or '未指定'}",
            f"- 大交通：{brief.transport_preference or '未指定'}",
            f"- 住宿：{brief.lodging_preference or '未指定'}",
            f"- 兴趣：{'、'.join(brief.interests) or '未指定'}",
