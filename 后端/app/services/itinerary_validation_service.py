...
def validate_itinerary(data: ItineraryData) -> None:
    ...
    for day in data.itinerary:
        if not day.id.strip():
            raise InvalidParamError("每日行程信息不完整")
        if day.id in seen_day_ids:
            raise InvalidParamError("每日行程存在重复项")
        ...
        for sch in day.schedules:
            if not sch.id.strip():
                raise InvalidParamError("行程条目信息不完整")
            if sch.id in seen_sch_ids:
                raise InvalidParamError("行程安排存在重复项")
            ...
            # Overnight transport (22:00 → 00:30) keeps HH:MM clocks; the end
            # belongs to the next calendar day and must be last on this date.
            end_timeline = timeline_end_minutes(start_m, end_m)
