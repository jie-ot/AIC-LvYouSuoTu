...
import { formatScheduleTimeRange, scheduleSpanMinutes } from "@/lib/schedule-time"
...
function stayLabel(start: string, end: string): string | null {
  const span = scheduleSpanMinutes(start, end)
...
function drawFooter(
  ...
  context.stroke()
  drawText(context, ["旅有所图"], PADDING, y + 22, font(18, 800), COLORS.muted, 26)
}
