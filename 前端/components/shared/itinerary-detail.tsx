...
export function ItineraryDetail({
  ...
  function savePendingMap() {
    ...
    void saveTravelImage(
      imageUrl,
      `${pendingSave.date}-${pendingSave.map.title}-路线图`,
      "每日路线图",
    )
      .then(() => {
        setPendingSave(null);
  ...
  function exportItinerary() {
    ...
    void renderItineraryImage(data)
      ...
      .catch((error: unknown) =>
        toast(
          error instanceof Error ? error.message : "长图保存失败，请稍后重试",
          "error",
        ),
      )
  ...
  return (
    <>
      {exportTarget ? createPortal(exportControl, exportTarget) : null}
      <article className={`itinerary-experience itinerary-mood-${mood}`}>
        ...
        {exportTarget === undefined ? exportControl : null}
        ...
        {(preparations.length > 0 ||
          ...
          <div className="itinerary-front-guides">
            ...
            {food_recommendations.length > 0 && (
              <GuideAccordion
                title="美食推荐"
                summary={`${food_recommendations.length} 项餐饮建议`}
                icon={Utensils}
                open={foodOpen}
            ...
            {itinerary.map((day, index) => {
              ...
              return (
                ...
                >
                  <span>D{index + 1}</span>
                  <strong>
                    {PERIOD_ICONS[day.schedules[0]?.time_period] || "✦"}
                  </strong>
            ...
            >
              ...
              {selectedDay.daily_maps?.length ? (
                <DailyMapCard
                  ...
                  onPreview={(map) =>
                    setPreviewMap({ map, date: selectedDay.date })
                  }
                  onRequestSave={(map) =>
                    setPendingSave({ map, date: selectedDay.date })
                  }
                />
              ) : null}
            </section>
  ...
  return (
    <section className={`daily-map-card ${open ? "is-open" : ""}`}>
      <button
        ...
        onClick={onToggle}
      >
        <span className="daily-map-toggle-icon">
          <MapIcon className="size-4" aria-hidden />
        </span>
        <span>
          <strong>当日路线图</strong>
          <small>
            {maps.length > 1 ? `${maps.length} 个路线区域 · ` : ""}
            {pointCount} 个地点
          </small>
        </span>
        <ChevronDown
          className={open ? "is-open size-4" : "size-4"}
          aria-hidden
        />
      </button>
...
function DailyMapPanel({
  ...
  const canShowImage =
    map.status === "ready" && Boolean(imageUrl) && failedImageId !== map.id;
  ...
  return (
    <div className="daily-map-panel">
      {canShowImage ? (
        ...
        <div
          className="daily-map-fallback"
          role="img"
          aria-label="地图图片暂时不可用"
        >
      ...
      <div className="daily-map-legend" aria-label="地图地点图例">
        {map.points.map((point) => (
          <span key={`${point.schedule_id}-${point.marker}`}>
            <b className={point.kind === "hotel" ? "is-hotel" : ""}>
              {point.marker}
            </b>
            {point.name}
          </span>
  ...
  return (
    <div className="daily-map-lightbox" role="presentation" onClick={onClose}>
      <div
        role="dialog"
        aria-modal="true"
        aria-label={`${selection.map.title}路线图大图`}
        onClick={(event) => event.stopPropagation()}
      >
        <div className="daily-map-lightbox-bar">
          <p>
            <strong>{selection.map.title}</strong>
            <span>{selection.date}</span>
          </p>
          <span>
            <button
              type="button"
              onClick={() => onRequestSave(selection)}
              aria-label="保存地图"
            >
              <Download className="size-4" aria-hidden />
            </button>
        ...
        <img
          src={imageUrl}
          alt={`${selection.map.title}当天酒店与景点相对位置大图`}
          draggable={false}
        />
      </div>
    </div>
...
function ScheduleCard({
  ...
  const hasDetails = Boolean(
    schedule.transport || schedule.distance_km || schedule.booking_required,
  );
  ...
  return (
    <li
      ...
      style={
        { "--schedule-index": index } as CSSProperties &
          Record<"--schedule-index", number>
      }
    >
...
function GuideAccordion({
  ...
  return (
    <section
      className={`itinerary-command ${tone === "food" ? "is-food" : ""}`}
    >
      <button
        type="button"
      ...
      >
        ...
        <ChevronDown
          className={open ? "is-open size-5" : "size-5"}
          aria-hidden
        />
      </button>
      {open ? (
        <div className="itinerary-command-content">{children}</div>
      ) : null}
    </section>
  );
}
