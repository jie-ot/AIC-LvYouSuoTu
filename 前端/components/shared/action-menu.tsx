  ...
  return (
    <>
      <button
        ...
        className="ui-icon-button flex size-11 items-center justify-center rounded-lg border border-border bg-card text-foreground"
        aria-label={label}
        aria-haspopup="menu"
        aria-expanded={open}
      >
      ...
      {open &&
        ...
        createPortal(
          <div
            ...
            className="ui-material-in z-50 overflow-hidden rounded-xl border border-border bg-popover py-1.5 shadow-lg"
          >
            {items.map((item) => (
              <button
                key={item.label}
